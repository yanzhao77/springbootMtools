...
public class CreateFile3 {
	...
	public static void main(String[] args) {
		...
		String pathListFile = "D:\\HulftConf\\XRSNDGO-42.txt";
		...
		String[] strArr= {"DD","UPDATE","COPY","BACKUP","RESTORE","PRINT","LISTDRTY","PUNCH","GENERATE","COMPARE","SCRATCH","RENAME","SETDRTY","UNLOAD","IF","GOTO","NOP","CALL","XCTL","MODE","ENDMODE"};
		for (File f : fileList) {
			readFileString(f, fileIds, arr);
		}
		/*
		for (int i = 0; i < arr.length; i++) {
			System.out.println(strArr[i]+"="+arr[i]);
			...
			while ((s = br.readLine()) != null) { // a 使用readLine方法，一次读一行
				...
				if (m.find()) {
					...
					*/
					...
					String nameAndFile = fileId+"="+ file.getName();
					System.out.println(nameAndFile);
					fileIds.add(nameAndFile);
				}
			}
			br.close();
		} catch (Exception e) {
