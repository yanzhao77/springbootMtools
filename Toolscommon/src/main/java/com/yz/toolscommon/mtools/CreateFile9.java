...
public class CreateFile9 {
    ...
    public static void main(String[] args) {
        ...
        String path = "D:\\itoutyuu\\01_移行前\\06_ソース一式\\正味資産_210930(20210929版)";
        ArrayList<File> fileList = new ArrayList<>();
        getFiles(path, fileList);
        List<String> stringSet = new ArrayList<>();
        for (File file : fileList) {
            if (file.getName().contains("S23D9021")){
        ...
        try {
            ...
            while ((s = br.readLine()) != null) { // a 使用readLine方法，一次读一行
                ...
                for (String byRegex : stringByRegex) {
                    ...
                    stringFileList.add(file.getName().replaceAll(".COB","") + "," + byRegex);
...
//        getFiles(Const.staticConfigPatch, fileList);
    ...
    public static void getFiles(String path, ArrayList<File> list) {
        ...
        if (file.isDirectory()) {
            ...
            for (File fileIndex : files) {
                ...
                } else if (fileIndex.getName().contains(".COB")) {
                    //如果文件是普通文件，则将文件句柄放入集合中
                    list.add(fileIndex);
                }
            }
        } else if (file.getName().contains(".COB")) {
            list.add(file);
        }
    }
    ...
    public static void writeFile(List<String> stringSet) {
        ...
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter(Const.result_pgm_sysin_txt));
            //输出到文件中
            orderList.forEach(f -> {
                try {
                    out.write(f + lineSep);
