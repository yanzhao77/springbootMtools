...
public class SqlCostInterceptor implements Interceptor{
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            logger.info("时间：[" + DateUtil.format(new Date(endTime), "yyyy-MM-dd HH:mm:ss.SSS")
                    + "],実行に時間がかかる：[" + sqlCost + "ms],SQL：[" + sql + "]");
            
            if(sqlCost>100) {
            	logger.warn("実行に時間がかかる>100ms");	
            }
    ...
    private String formatSql(String sql, Object parameterObject, List<ParameterMapping> parameterMappingList) {
        ...
        // 不传参数的场景，直接把Sql格式化一下返回出去
        if (parameterObject == null || CollUtil.isEmpty(parameterMappingList)) {
            return sql;
        }
        ...
        try {
            if (CollUtil.isNotEmpty(parameterMappingList)) {
