...
public class Math {
    public void execMath() {
        CharStream input = CharStreams.fromString("12*2+24\r\n");
        MathLexer lexer = new MathLexer(input);
        lexer.getAllTokens().forEach( token -> System.out.println(token.getText()));
        for (Token allToken : lexer.getAllTokens()) {
            System.out.println(allToken.getText());
        }
